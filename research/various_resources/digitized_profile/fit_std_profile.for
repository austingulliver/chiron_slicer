...
!******************************************************
!np             : Number of points for an order in the cross dispersion direction
!irow_low (int) :  the reference to the LOW ccd order row
!irow_high (int) the reference to the HIGH ccd order row
!wt :  is an array of weights the correspondig pixel in the cross dispersion direction 
!******************************************************

      npt=irow_high-irow_low+1

!******************************************************    
! Seems to be doing some shifting in the pixels in the cross dispersion direction
! defines y_temp seems to be new to values to be used to fit a Fourier Transform 
!******************************************************
...
!******************************************************
!JL : Opens the file that has ? the point spread function. 
!******************************************************
...
!******************************************************
!JL : Manipulating some high intensity pixels from
!******************************************************
...
c Do not include 0 wt points
      do j=1,npt  ! npt are the number of point  in the cross dispersion direction for a given order 
          if(wt(j).ne.0.)then
              if(y_temp(j).gt.half_intensity)then ! iterating over the cross dispersion pixels and MANIPULATING the ones greater than  Full Width - Half Maximmum 
                  iy_lo_fwhm=j-1
...
!******************************************************
!JL : Used as part of prev process 
!******************************************************
c3120 do j=js,npt
 3120 do j=npt,js,-1
          if(wt(j).ne.0.)then
...
c     end do
      ...
      fit_type='Psf'
!******************************************************
!JL : All prev code is for set up the values for curfit 
!******************************************************
!x :       the x values 
!y_temp :  the y values
!wt_temp :  the weights 
!npt : number of points in the cross dispersion direction for a give column 
!ntermm: number of terms =6 (5th Order Polynomial + constant Term)
!mode: curfit has 5 modes:   see https://www.osti.gov/biblio/6343557-curfit-curve-fitting-experimental-data-points-cdc6600-fortran-iv-ascent
!                         (1) polynomial y = summation over i of a(i)*(x**(i))
!par_values : (input and output )parameter values 

!deltaa : (input) passing dummy values of 0
!sigma_values : (output?) of the error in y 
!flambda :   = 0.001
!yfit_temp : (output?)  of the fitted values
!chisqr : (output? )Ki Squared  of the fitted values 
...
c 
c
c Temporarily set rms_temp to chisqr for omc_ratio calculation   : WHAT IS OMC ? 
      rms_temp=chisqr
