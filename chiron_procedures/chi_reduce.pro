...
lfn = '/nfs/morgan/chiron/tous/mir7/logsheets/'+strt(date)+'.log'
...
spawn, 'ls -1 /nfs/morgan/chiron/tous/mir7/thid/thidfile/'+run+'.*', res
...
;retrieve the thid files already processed:
spawn, 'ls -1 /nfs/morgan/chiron/tous/mir7/thid/thidfile/'+lastrun+'.*', res
...
initthar=0
lastrunflag=1
dumim = readfits('/nfs/morgan/chiron/tous/mir7/fitspec/'+lastrun+'.'+strt(lress)+'.fits', hd, /silent)
endif;res=''
...
dumim = readfits('/nfs/morgan/chiron/tous/mir7/fitspec/'+run+'.'+strt(lress[tharidx], f='(I04)')+'.fits', hd, /silent)
ii++
...
if ~lastrunflag then begin
  restore, '/nfs/morgan/chiron/tous/mir7/thid/thidfile/'+run+'.'+strt(obsnum[i-1], f='(I04)')+'.thid'
endif
if (lastrunflag and (i eq 1)) then begin
  restore, '/nfs/morgan/chiron/tous/mir7/thid/thidfile/'+lastrun+'.'+strt(obsnum[i-1], f='(I04)')+'.thid'
endif
  rdsk, thar, '/nfs/morgan/chiron/tous/mir7/iodspec/'+run+'.'+strt(obsnum[i], f='(I04)'), 1
  
  print, 'NOW CALCULATING THE WAVELENGTH SOLUTION FOR: ', $
  				'/nfs/morgan/chiron/tous/mir7/iodspec/'+run+'.'+strt(obsnum[i], f='(I04)')
  ...
  save, thid, file='/nfs/morgan/chiron/tous/mir7/thid/thidfile/'+run+'.'+strt(obsnum[i], f='(I04)')+'.thid'
  
  mkwave, w, thid.wvc
  
  w = reverse(w, 2)
  
  save, w, file='/nfs/morgan/chiron/tous/mir7/thid/wavfile/ctio_'+run+'.'+strt(obsnum[i], f='(I04)')+'.dat'
...
qbarylog, '/nfs/morgan/chiron/tous/mir7/logsheets/'+date+'.log', prefix=run
