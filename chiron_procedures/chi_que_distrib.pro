...
qodir = '/nfs/morgan/chiron/tous/mir7/queueobs/'+date+'/'

logpath='/nfs/morgan/chiron/tous/mir7/logsheets/'
logsheet=logpath+'20'+strmid(date, 0, 2)+'/'+date+'.log'
fits_path='/nfs/morgan/chiron/tous/mir7/fitspec/'
...
for i=long(startqtz), long(endqtz) do $;
	spawn, 'cp -vf /nfs/morgan/chiron/raw/mir7/'+date+'/'+prefix[0]+'.'+strt(i, f='(I04)')+'.fits '+caldir
endfor
...
for ii=0, numthars-1 do begin
	spawn, 'cp -vf /nfs/morgan/chiron/raw/mir7/'+date+'/'+prefix[0]+'.'+strt(obnm[tharnums[ii]])+'.fits '+caldir
endfor
...
for ii=0, numbias-1 do begin
	spawn, 'cp -vf /nfs/morgan/chiron/raw/mir7/'+date+'/'+prefix[0]+'.'+strt(obnm[biasnums[ii]])+'.fits '+caldir
endfor
...
for ii=0, numiod-1 do begin
	spawn, 'cp -vf /nfs/morgan/chiron/raw/mir7/'+date+'/'+prefix[0]+'.'+strt(obnm[iodnums[ii]])+'.fits '+caldir
    spawn, 'cp -vf /nfs/morgan/chiron/tous/mir7/fitspec/'+date+'/a'+prefix[0]+'.'+strt(obnm[iodnums[ii]])+'.fits '+caldir
endfor
...
spawn, 'cp -vf /nfs/morgan/chiron/tous/mir7/flats/chi'+date+'.fiberflat.fits '+caldir
spawn, 'cp -vf /nfs/morgan/chiron/tous/mir7/flats/chi'+date+'.narrowflat.fits '+caldir
spawn, 'cp -vf /nfs/morgan/chiron/tous/mir7/flats/chi'+date+'.slicerflat.fits '+caldir
spawn, 'cp -vf /nfs/morgan/chiron/tous/mir7/flats/chi'+date+'.slitflat.fits '+caldir
endif;KW:nocalib
;stop
...
for pi=0, n_elements(proparr)-1 do begin
	...
	if plct gt 0 then begin
		...
		for flnbr=0, plct-1 do begin
			spawn, 'cp -vf /nfs/morgan/chiron/raw/mir7/'+date+'/'+prefix+'.'+obnm[pilocs[flnbr]]+'.fits '+plandir
			spawn, 'cp -vf /nfs/morgan/chiron/tous/mir7/fitspec/'+date+'/r'+prefix+'.'+obnm[pilocs[flnbr]]+'.fits '+plandir
			spawn, 'cp -vf /nfs/morgan/chiron/tous/mir7/fitspec/'+date+'/a'+prefix+'.'+obnm[pilocs[flnbr]]+'.fits '+plandir
		endfor
		endif;KW(nocopy)
		if ~keyword_set(nolog) then begin
			...
			print, '********************************************'
			;LogSheetFileName
			lsfnm = '/nfs/morgan/chiron/tous/mir7/logsheets/20'+strmid(date, 0, 2)+'/'+date+'.log'
			print, lsfnm
