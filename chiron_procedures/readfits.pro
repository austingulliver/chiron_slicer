...
function READFITS, filename, header, heap, NOSCALE = noscale, $
  ...
  ;print, 'Actual version of READFITS'
...
; Set default keyword values
   ...
   silent =1 ;  added to make script don't print anythin.  Remove if needed 
...
; Use "TEMPORARY" function to speed processing.  
   ...
   if do_scale then begin
          ...
          Bscale = double( sxpar( header, 'BSCALE' , Count = N_bscale))
          Bzero = double( sxpar(header, 'BZERO', Count = N_Bzero ))
