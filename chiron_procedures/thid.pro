...
;Useful sizes.
  ...
  print, 'THID: Number of pixels: '+ string(npix)
  print, 'THID: Number of orders: '+ string(nord)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if cmd eq 'a' then begin
      ...
      for i=0, nord-1 do begin
        onum = olist(i)
        print, ' -->> THID: Reading Physical Order ' +string(onum)
        wbeg = min(wave(*,i), max=wend)
        iwhr = where(th_wair ge wbeg and th_wair le wend, nwhr)
        ;;;;;;;;;;;;;;;;;;;;;;;;;;
        ;JL START    ;    JL (1/3)
        ...
        ;print, '------', onum
        flag=0
        targetOrder= 1380 ;This has to make referece to physical order.  Tying an impossible order number will makescript dont plot anything 
        if (onum eq targetOrder) then begin
          flag=1
          myCounter=0
        endif
        if (flag eq 1) then begin
          selected =th_wair(iwhr)
          PRINT, 'WAVELENGTHS (RANGE ) -----------------------------'
          print, selected
          PRINT, 'WAVELENGTHS (RANGE ) -----------------------------'
          PRINT,'For the TARGET order there are ', STRTRIM(nwhr, 2),' compared lines'
          x=wave(*,i )
          y=s(*,i) ; Has already been scaled
          title= 'Physical Order :'+ string(onum)
          plt=plot(x,y,YTITLE='Flux',XTITLE='Pixels',  TITLE=title)
        endif
        ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
        ;JL END
        ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;          th_pix = (interpol(findgen(npix), congrid(wave(*,i),npix), th_wav))(0)
          ...
          if ipix gt awin and ipix lt (npix-awin-1) then begin
            ...
            if amp gt minamp then begin
              ...
              if abs(gpar(1) - th_pix) le maxres and $
                ...
                nm = nm + 1
                myColor = 'green' 
                lineCounter =lineCounter +1 ; added by JL   
              endif
            endif
          endif
          ;;;;;;;;;;;;;;;;;;;;
          ;JL START    JL (2/3)
          ;;;;;;;;;;;;;;;;;;;;
          ;print, ' * Total number of lines found for order  '+string(onum) + '    are   ' + string( lineCounter )
          if flag eq 1 then begin
            ...
            liney=[miny,maxy]
            linex=[th_wav,th_wav]
            plt2=plot(linex,liney,color=myColor,/overplot)   ; added by JL FOR TESTING
        ...
        ; JL START        ;    JL (3/3)
        ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
        print, ' * Total number of lines found up to order  :'+ string(onum) +'  is : '+ string(lineCounter)
        ; Reseting back to 0

        If flag eq 1 then begin
          plt=plot(x,y,/overplot)
          print, 'Number of lines found for the plotted order :',myCounter
