...
;+
  ...
  ;-
function multuply_cons, array, constants
  compile_opt idl2
  
  for order = 0L, n_elements(constants)-1 do begin
    array[*, order] = array[*, order] * constants[order] 
  endfor
...
function remove_cr_by_sigma, paths, sigma
  ...
  for idx = 0L, n_elements(paths)-1 do begin
    ...
    avgs.add,  mean(spectrum, dimension=1) ; Transforms to [1, 73] pr [73]
  endfor
  ...
  avgs = avgs.toarray() ; transforms from a list([74],[1 for each order ]....,[74] ) to ->  [ [74], [74], [74] . . .  [74] ] up to the normal of obersevarion that exist
                        ; E .g. 12 x 73
  avg  = mean(avgs, dimension=1) ;  VERIFY !!!!  thre should be 1 for each order -> [avg1, avg2 , ....  avg74]
  ...
  for idx = 0L, n_elements(paths)-1 do begin
    ...
    goldenFactor   = avg / mean(spectrum, dimension=1) ; For given spectra observation. I'm manipulating all orders at the same time.
    ...
    spectra [*,*,idx] = multuply_cons(spectrum,goldenFactor ) ; Tricky manipulation of arrays cause   [4112,73] x [73]
  endfor
  ...
  for idx = 0L, n_elements(paths)-1 do begin
    ...
    ; y= y /goldenFactors[idx] ; to return to original intensity
